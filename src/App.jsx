...
function FieldOpt({ label, children }) { return <label className="block"><span className="mb-1 block text-xs font-medium text-slate-400">{label} <span className="font-normal text-emerald-600">(opcional)</span></span>{children}</label>; }
        ...
        {empieza.length === 0 ? (
          ...
          <ul className="space-y-2">
            {empieza.map((u) => {
              ...
              return (
                <li key={u.key} onClick={() => onOpen(u.actId)} style={{ borderLeftColor: PRIO[u.prioridad].hex }} className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-l-4 border-slate-200 bg-white px-3 py-2.5 hover:border-slate-300">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-slate-800">{u.titulo}</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        {esperando.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-100">
            {esperando.map((u) => (
              <li key={u.key + u.tag} onClick={() => onOpen(u.actId)} className="flex cursor-pointer items-center justify-between gap-3 py-2.5 hover:bg-slate-50">
                <div className="min-w-0">
                  <p className="truncate text-sm text-slate-700">{u.titulo}</p>
            ...
            <div key={g.id} className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="mb-3 flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2.5">
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500"><User size={15} /></span>
                  <div><p className="text-sm font-semibold text-slate-800">{g.nombre}</p><p className="text-[11px] text-slate-400">{g.rol}</p></div>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700" title="Completadas en el rango">{g.historial.length} hechas</span>
                  <span className="rounded-full bg-slate-900 px-2 py-0.5 text-xs font-medium text-white" title="Abiertas">{g.abiertas} abiertas</span>
                </div>
              </div>
              <ul className="space-y-2">
                {lista.map((u) => (
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          {cat.personas.map((p, i) => (
            <div key={p.id} className="flex items-center gap-2">
              <input className="flex-1 rounded border border-slate-200 px-2 py-1 text-sm" value={p.nombre} onChange={(e) => { const a = [...cat.personas]; a[i] = { ...p, nombre: e.target.value }; upd({ personas: a }); }} />
              <input className="w-44 rounded border border-slate-200 px-2 py-1 text-xs text-slate-500" value={p.rol} onChange={(e) => { const a = [...cat.personas]; a[i] = { ...p, rol: e.target.value }; upd({ personas: a }); }} />
              <button onClick={() => upd({ personas: cat.personas.filter((x) => x.id !== p.id) })} className="rounded p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 size={14} /></button>
            </div>
          ))}
        </div>
        <div className="mt-3 flex items-center gap-2 border-t border-slate-100 pt-3">
          <input className="flex-1 rounded border border-slate-200 px-2 py-1 text-sm" placeholder="Nombre" value={nP.nombre} onChange={(e) => setNP({ ...nP, nombre: e.target.value })} />
          <input className="w-44 rounded border border-slate-200 px-2 py-1 text-xs" placeholder="Rol" value={nP.rol} onChange={(e) => setNP({ ...nP, rol: e.target.value })} />
          <button onClick={() => { if (nP.nombre.trim()) { upd({ personas: [...cat.personas, { id: "u" + Date.now(), nombre: nP.nombre, rol: nP.rol }] }); setNP({ nombre: "", rol: "" }); } }} className="inline-flex items-center gap-1 rounded-md bg-slate-900 px-3 py-1.5 text-xs font-medium text-white"><Plus size={13} />Agregar</button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {cat.proyectos.map((p, i) => (
            <div key={p.id} className="flex items-center gap-2">
              <input className="flex-1 rounded border border-slate-200 px-2 py-1 text-sm" value={p.nombre} onChange={(e) => { const a = [...cat.proyectos]; a[i] = { ...p, nombre: e.target.value }; upd({ proyectos: a }); }} />
              <select className="w-40 rounded border border-slate-200 px-2 py-1 text-xs" value={p.entidad} onChange={(e) => { const a = [...cat.proyectos]; a[i] = { ...p, entidad: e.target.value }; upd({ proyectos: a }); }}>{ENTIDADES.map((x) => <option key={x}>{x}</option>)}</select>
              <button onClick={() => upd({ proyectos: cat.proyectos.filter((x) => x.id !== p.id) })} className="rounded p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 size={14} /></button>
            </div>
          ))}
        </div>
        <div className="mt-3 flex items-center gap-2 border-t border-slate-100 pt-3">
          <input className="flex-1 rounded border border-slate-200 px-2 py-1 text-sm" placeholder="Nombre del proyecto" value={nPr.nombre} onChange={(e) => setNPr({ ...nPr, nombre: e.target.value })} />
          <select className="w-40 rounded border border-slate-200 px-2 py-1 text-xs" value={nPr.entidad} onChange={(e) => setNPr({ ...nPr, entidad: e.target.value })}>{ENTIDADES.map((x) => <option key={x}>{x}</option>)}</select>
          <button onClick={() => { if (nPr.nombre.trim()) { upd({ proyectos: [...cat.proyectos, { id: "p" + Date.now(), ...nPr }] }); setNPr({ nombre: "", entidad: "Primer Equipo", descripcion: "" }); } }} className="inline-flex items-center gap-1 rounded-md bg-slate-900 px-3 py-1.5 text-xs font-medium text-white"><Plus size={13} />Agregar</button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {plantillas.map((t, i) => (
            <div key={t.id} className="flex flex-wrap items-center gap-2 rounded-lg bg-slate-50 p-2">
              <input className="min-w-[140px] flex-1 rounded border border-slate-200 px-2 py-1 text-sm" value={t.nombre} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, nombre: e.target.value }; upd({ plantillasTarea: a }); }} />
              <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={t.entidad} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, entidad: e.target.value }; upd({ plantillasTarea: a }); }}>{ENTIDADES.map((x) => <option key={x}>{x}</option>)}</select>
              <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={t.area} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, area: e.target.value, subcategoria: "" }; upd({ plantillasTarea: a }); }}>{AREAS.map((x) => <option key={x}>{x}</option>)}</select>
              <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={t.tipoTrabajo} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, tipoTrabajo: e.target.value }; upd({ plantillasTarea: a }); }}>{cat.tipos.map((x) => <option key={x}>{x}</option>)}</select>
              <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={t.prioridad} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, prioridad: e.target.value }; upd({ plantillasTarea: a }); }}>{PRIOS.map((x) => <option key={x}>{x}</option>)}</select>
              <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={t.responsableId} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, responsableId: e.target.value }; upd({ plantillasTarea: a }); }}>{cat.personas.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select>
              <label className="inline-flex items-center gap-1 text-[11px] text-slate-500"><input type="checkbox" checked={!!t.conSubtareas} onChange={(e) => { const a = [...plantillas]; a[i] = { ...t, conSubtareas: e.target.checked }; upd({ plantillasTarea: a }); }} className="h-3.5 w-3.5" />proceso</label>
              <button onClick={() => upd({ plantillasTarea: plantillas.filter((x) => x.id !== t.id) })} className="rounded p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 size={14} /></button>
            </div>
          ))}
          {plantillas.length === 0 && <p className="py-2 text-center text-xs text-slate-400">Sin plantillas aún.</p>}
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
          <input className="min-w-[140px] flex-1 rounded border border-slate-200 px-2 py-1 text-sm" placeholder="Nombre de la plantilla" value={nPl.nombre} onChange={(e) => setNPl({ ...nPl, nombre: e.target.value })} />
          <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={nPl.entidad} onChange={(e) => setNPl({ ...nPl, entidad: e.target.value })}>{ENTIDADES.map((x) => <option key={x}>{x}</option>)}</select>
          <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={nPl.area} onChange={(e) => setNPl({ ...nPl, area: e.target.value })}>{AREAS.map((x) => <option key={x}>{x}</option>)}</select>
          <select className="rounded border border-slate-200 px-1.5 py-1 text-[11px]" value={nPl.responsableId} onChange={(e) => setNPl({ ...nPl, responsableId: e.target.value })}>{cat.personas.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select>
          <label className="inline-flex items-center gap-1 text-[11px] text-slate-500"><input type="checkbox" checked={nPl.conSubtareas} onChange={(e) => setNPl({ ...nPl, conSubtareas: e.target.checked })} className="h-3.5 w-3.5" />proceso</label>
          <button onClick={() => { if (nPl.nombre.trim()) { upd({ plantillasTarea: [...plantillas, { id: "t" + Date.now(), ...nPl }] }); setNPl({ ...nPl, nombre: "" }); } }} className="inline-flex items-center gap-1 rounded-md bg-slate-900 px-3 py-1.5 text-xs font-medium text-white"><Plus size={13} />Agregar</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen w-full bg-slate-50">
      <div className="w-full px-2 py-2" style={{ background: NAVY }}>
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-2">
          <img src={LOGO} alt="Guayaquil City F.C." className="h-9 w-9" />
          <div className="flex-1">
            <h1 className="text-base font-semibold leading-tight text-white">Guayaquil City F.C.</h1>
            <p className="text-[11px] uppercase tracking-widest" style={{ color: "#7fb3e0" }}>Centro de Mando</p>
          </div>
          ...
          <button onClick={() => setQuick(true)} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-white/90 hover:bg-white/10"><Clock size={15} />Rápida</button>
          <button onClick={() => setModal({ a: null })} className="inline-flex items-center gap-1.5 rounded-lg bg-white/10 px-3 py-2 text-sm font-medium text-white hover:bg-white/20"><Plus size={16} />Nueva</button>
          <button onClick={onSignOut} title={session?.user?.email || "Cerrar sesión"} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-2 text-xs font-medium text-white/70 hover:bg-white/10 hover:text-white">Salir</button>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6">
        ...
        <div className="mb-4 flex items-center gap-1 rounded-lg border border-slate-200 bg-white p-0.5 w-fit">
          {["Todos", ...ENTIDADES].map((s) => (
            <button key={s} onClick={() => setEntidad(s)} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition ${entidad === s ? "text-white" : "text-slate-500 hover:text-slate-800"}`} style={entidad === s ? { backgroundColor: s === "Todos" ? NAVY : ENT_COLOR[s] } : {}}>
